...
import { probeMicrophone } from './services/audioCapture'
import {
...
const sourceOptions: AudioSource[] = ['microphone']
...
function App() {
  ...
  async function probeMicrophoneInput() {
  ...
  function waitForAsrIdle(meetingId: string): Promise<void> {
    ...
    return new Promise((resolve) => {
      ...
      const timeoutId = window.setTimeout(() => {
        const pendingResolvers = asrIdleResolversRef.current.get(meetingId) ?? []
        asrIdleResolversRef.current.set(
          meetingId,
          pendingResolvers.filter((pendingResolve) => pendingResolve !== finishWait),
        )
        resolve()
      }, 15_000)
      const finishWait = () => {
  ...
  function handleStreamingAsrFailure(sessionId: string, message: string) {
    ...
    finishStreamingAsrLocally(sessionId, 'error')
  }

  function finishStreamingAsrLocally(sessionId: string, outcome: 'finished' | 'error' = 'finished') {
    const session = streamingSessionsRef.current.get(sessionId)
    if (!session) {
      return
    }
      ...
      <main className="workspace">
        <header className="topbar">
          ...
          <div className="capture-actions">
            <button type="button" className="ghost-button" onClick={probeMicrophoneInput}>
              <Mic size={17} />
              <span>Mic probe</span>
            </button>
          </div>
        ...
        <section className="metrics">
          ...
          <Metric icon={Database} label="Saved segments" value={segments.length.toString()} />
